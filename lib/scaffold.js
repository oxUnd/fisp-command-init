...
module.exports = {
    ...
    download: function(m, dist, options, cb) {
        ...
        handle.install(cur, comp, {deps: options.deps, overwrite: true}, function (err, installed) {
            if (err) {
                fis.log.error(err);
                return;
            }
    ...
    list: function (options) {
        ...
        handle.search(query, function (err, res) {
            if (err) {
                fis.log.error(err);
                return;
            }
            ...
            res.forEach(function (c) {
                ...
                process.stdout.write(
                    (c.name + '@' + c.version).blue 
                    + ' ' 
                    + c.maintainers[0].name.green 
                    + ' ' 
                    + c.description.grey + '\n'
                );
            });
